...
require "open3"
require "socket"
load Rails.root.join("bin/taskrail")

RSpec.describe "bin/taskrail" do
  ...
  def run_cli(api_url, *args, env: {})
    Open3.capture3({ "TASKRAIL_API_URL" => api_url }.merge(env), Rails.root.join("bin/taskrail").to_s, *args)
  end

  it "execs the Ink TUI when no subcommand is given" do
    api_url = "http://api.example.test"
    cli = TaskRailCli.new([], { "TASKRAIL_API_URL" => api_url }, StringIO.new, StringIO.new)
  ...
  it "passes TUI options through when args are given without a CLI subcommand" do
    api_url = "http://api.example.test"
    cli = TaskRailCli.new(["--queue", "ops", "--refresh", "10"], { "TASKRAIL_API_URL" => api_url }, StringIO.new, StringIO.new)
  ...
  it "renders a one-shot dashboard" do
    ...
    with_server(responses) do |api_url, requests|
      ...
      expect(status).to be_success
      expect(stdout).to include("TaskRail Dashboard")
  ...
  it "watches dashboard output for a capped number of iterations" do
    ...
    with_server(responses) do |api_url, requests|
      stdout, _stderr, status = run_cli(
        ...
        env: { "TASKRAIL_DASHBOARD_ITERATIONS" => "2" }
      )

      expect(status).to be_success
      expect(stdout.scan("TaskRail Dashboard").size).to eq(2)
