...
RSpec.describe "security scan cookbook", type: :request do
  def create_fake_security_scan_queue
    queue = WorkQueue.create!(
      name: "Security Scan Fixture #{SecureRandom.hex(4)}",
      slug: "security-scan-fixture-#{SecureRandom.hex(4)}",
      stages: %w[scan_vulnerabilities classify_severity draft_fixes run_tests human_review done],
      config: { "default_max_retries" => 0, "max_regression_loops" => 0 }
    )
    queue.stage_configs.create!(stage_name: "scan_vulnerabilities", adapter_type: "fake", completion_criteria: ["scan_completed"])
  ...
  it "drives a work item through all stages using the fake adapter" do
    queue = create_fake_security_scan_queue
    post "/api/v1/work_items", params: { queue: queue.slug, title: "Run security scan", spec_url: "docs/specs/cookbook-security-scan.md" }
    ...
    expect(work_item.artifacts.pluck(:kind)).to include("vulnerability_scan", "severity_report", "fix_patches", "test_results")
  end
end
