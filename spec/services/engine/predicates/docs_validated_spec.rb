...
RSpec.describe Engine::Predicates::DocsValidated do
  ...
  it "passes when latest success report uses generic artifact wrapper" do
    claim = build_claim(reports: [
      { status: :success, body: { "artifact_kind" => "validation_results", "artifact" => { "valid" => true, "errors" => [] } } }
    ])
  ...
  it "uses the newest success report by creation time" do
    claim = build_claim(reports: [
      { status: :success, body: { "validation_results" => { "valid" => false, "errors" => [] } }, created_at: 2.minutes.ago },
      { status: :success, body: { "validation_results" => { "valid" => true, "errors" => [] } }, created_at: 1.minute.ago }
    ])
  ...
  it "ignores newer non-success reports" do
    claim = build_claim(reports: [
      { status: :success, body: { "validation_results" => { "valid" => true, "errors" => [] } }, created_at: 2.minutes.ago },
      { status: :failure, body: { "validation_results" => { "valid" => false, "errors" => ["newer failure"] } }, created_at: 1.minute.ago }
    ])
