...
- `Adapters::ShellScriptAdapter` defaults `working_directory` to `Rails.root.to_s`; do not hardcode `/Users/gregmushen/work/code/stupidclaw` in queue YAML.
...
**Objective:** Provide a tiny deliberately under-tested app surface for scan/gap/generation fixtures without depending on StupidClaw's current incidental coverage gaps.
...
Queue YAML and prompt references are relative to `Rails.root`. The `shell_script` stages intentionally omit `working_directory` so `Adapters::ShellScriptAdapter` uses its `Rails.root` default. Do not add absolute paths such as `/Users/gregmushen/work/code/stupidclaw` to queue YAML or fixtures.
