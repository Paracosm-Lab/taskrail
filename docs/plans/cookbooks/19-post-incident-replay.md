...
**Goal:** Add the `post_incident_replay` cookbook queue so StupidClaw can ingest incident evidence, reconstruct a timeline, separate root cause from symptoms, grade the response, draft runbook/alert/code/process improvements, and stop at human review.
...
- Do not hardcode `/Users/gregmushen/work/code/stupidclaw` or any absolute repository path in queue YAML, prompts, specs, fixtures, docs, or implementation code.
...
Create `cookbooks/fixtures/incidents/ops_pipeline_p1/` as a local replay of the StupidClaw ops pipeline E2E incident.
