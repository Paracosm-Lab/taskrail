...
**Goal:** Add the `incident_readiness` cookbook queue so StupidClaw can inventory services, score operational readiness, identify gaps, draft improvements, and stop for human review with a standalone readiness scorecard.
...
**Tech Stack:** Rails, RSpec, YAML queue seeds, StupidClaw `WorkQueue` / `StageConfig` models, `Engine::PredicateRegistry`, `Engine::PredicateResult`, `inline_claude` adapter, fake gate stages.
...
- Work from repository root: `/Users/gregmushen/work/code/stupidclaw`.
...
- Use Greg's Mac rbenv command prefix for focused specs:
  ...
  ```
- Keep queue YAML portable. Do not hardcode `/Users/gregmushen/work/code/stupidclaw` or any absolute repo path in YAML, prompt files, seeds, or fixtures.
...
```bash
rg "/Users/gregmushen/work/code/stupidclaw" config/queues/incident_readiness.yml prompts/readiness_*.md spec/fixtures/incident_readiness docs/cookbooks/incident-readiness-scoring.md
```
