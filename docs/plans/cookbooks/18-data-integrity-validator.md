...
**Goal:** Add the `data_integrity` cookbook queue so StupidClaw can derive database integrity rules from a Rails app, scan the database with read-only SQL, assess violation impact, draft dry-run-first repair scripts, and stop at a destructive-data human review gate.
...
- Do not hardcode `/Users/gregmushen/work/code/stupidclaw`, `/Users/`, `Rails.root.to_s`, or any absolute checkout path in queue YAML, prompts, specs, fixtures, scripts, or implementation code.
...
stage_configs:
  ...
  human_review:
    ...
    agent_prompt: DATA REPAIRS ARE DESTRUCTIVE. Review every generated repair script, run dry-run mode first, verify before/after counts, inspect prevention migrations separately, then approve or reject manual execution. StupidClaw must not execute repairs automatically.
...
Static Rails-shaped fixture for the StupidClaw Data Integrity Validator cookbook.
