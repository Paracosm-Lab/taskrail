...
```bash
ruby -e 'paths = Dir["config/queues/chaos_*.yml"] + Dir["prompts/{chaos,response}_*.md"] + Dir["docs/cookbooks/09-chaos-monkey.md"]; bad = paths.select { |p| File.read(p).include?("/Users/gregmushen/work/code/stupidclaw") }; abort("hardcoded paths: #{bad.join(", ")}") unless bad.empty?; puts "portable paths ok"'
```
