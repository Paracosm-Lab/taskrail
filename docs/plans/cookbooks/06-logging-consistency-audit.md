...
**Tech Stack:** Rails, RSpec, YAML seed loader with `file://` prompt resolution, StupidClaw `WorkQueue`/`StageConfig`/`Artifact` models, `Engine::PredicateRegistry`, `Engine::Predicates::*`, `Adapters::ShellScriptAdapter`.
...
```bash
grep -R "/Users/gregmushen/work/code/stupidclaw\|file:///." config/queues/logging_audit.yml prompts/logging_*.md
```
...
grep -R "/Users/gregmushen/work/code/stupidclaw\|file:///." \
  config/queues/logging_audit.yml \
  prompts/logging_scan_statements.md \
  prompts/logging_assess_quality.md \
  prompts/logging_draft_standard.md \
