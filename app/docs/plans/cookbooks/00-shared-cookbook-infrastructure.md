...
**Goal:** Build the shared `cookbooks/` directory, fake Docker Compose services, README/run instructions, and verification guardrails that every TaskRail cookbook example can reuse.
...
Do not add a repo-root `working_directory` to any queue YAML as part of this task. Compose relative paths resolve from the Compose file directory, and the TaskRail adapter already runs from `Rails.root`.
