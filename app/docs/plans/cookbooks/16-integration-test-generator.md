...
**Goal:** Add the `integration_tests` cookbook queue so TaskRail can map critical end-to-end user flows, identify integration boundaries, generate runnable integration specs, run them, and gate the results through human review.

**Architecture:** This follows TaskRail's seeded cookbook architecture: add portable queue YAML under `config/queues/`, keep long prompts in repo-relative prompt files, add two artifact predicates under `Engine::Predicates`, reuse and slightly broaden `tests_generated` so it accepts the cookbook's `integration_specs` artifact kind, and cover everything with focused RSpec seed/predicate/E2E specs. The deterministic E2E fixture uses TaskRail itself as the integration target and avoids real Claude/Docker calls by adding cookbook-aware fake adapter outputs for the exact stage names.
...
- Keep fake/docker-friendly infrastructure local and deterministic. Do not add cookbook-specific Docker Compose services; use TaskRail's Rails app, the existing fake adapter, and a spec fixture flow.
...
```bash
grep -R "/Users/gregmushen/work/code/taskrail\|file:///.\|working_directory:" config/queues/integration_tests.yml prompts/integration_*.md
```
...
**Objective:** Make the cookbook E2E spec able to exercise TaskRail's real API, engine runner, adapter boundary, report/artifact persistence, predicates, and stage advancement without invoking real Claude or shell commands.
...
### Task 11: Add TaskRail self-integration E2E spec
...
```bash
! grep -R "/Users/gregmushen/work/code/taskrail\|file:///.\|working_directory:" \
  config/queues/integration_tests.yml \
...
- [ ] E2E spec uses TaskRail itself: API creates work item, engine tick claims stages, fake adapter runs, reports/artifacts are stored, predicates pass, stage advances to `done`.
