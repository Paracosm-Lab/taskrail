...
**Goal:** Turn the original TaskRail MVP development queue into a cookbook example that demonstrates intake, decomposition, async Codex build, shell validation, review regression, CLI visibility, and a Docker-friendly fixture feature workflow.
...
**Tech Stack:** Rails 8, ActiveRecord, RSpec, YAML queue seeds, `db/seeds.rb` `file://` prompt resolver, `Engine::Runner`, `Engine::TransitionManager`, `CodexAdapter`, `ShellScriptAdapter`, `InlineClaudeAdapter`, `CheckAsyncClaimsJob`, `bin/taskrail`, Docker-friendly cookbook fixture apps.

**Source spec:** `/Users/gregmushen/docs/superpowers/specs/2026-05-04-taskrail-design.md`, especially Section 6 “The Engine Loop”, Section 8 “Queue Configuration — MVP Development Queue”, and Section 10 “CLI”.
...
- `Adapters::ShellScriptAdapter` defaults command execution to `Rails.root.to_s`; do not put `/Users/gregmushen/work/code/taskrail` or any other absolute checkout path into queue YAML.
...
Small Ruby fixture app for the TaskRail feature development cookbook.
...
- Exercise existing: `bin/taskrail`
...
```ruby
  it "documents the feature development cookbook workflow and CLI commands" do
    ...
    expect(content).to include("taskrail submit --queue development-codex")
    expect(content).to include("taskrail status")
    expect(content).to include("taskrail answer")
...
**Files:**
- Modify: `spec/cli/taskrail_spec.rb`
- Exercise existing: `bin/taskrail`

**Step 1: Write failing or characterization spec**

Add this example near the existing submit/list command examples in `spec/cli/taskrail_spec.rb`:
...
```bash
PATH="$HOME/.rbenv/shims:$HOME/.rbenv/bin:$PATH" bundle exec rspec spec/cli/taskrail_spec.rb --format documentation
```
...
Update `bin/taskrail` so `submit` accepts `--queue development-codex`, posts the queue slug unchanged, and prints the returned id. Do not special-case cookbook queues.
...
```bash
git add spec/cli/taskrail_spec.rb bin/taskrail
git commit -m "test: cover feature development cookbook CLI submit"
```

If no production code changed, omit `bin/taskrail` from `git add`.
