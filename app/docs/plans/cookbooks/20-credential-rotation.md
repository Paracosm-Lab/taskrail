...
**Goal:** Add the `credential_rotation` cookbook queue so TaskRail can inventory secrets, map dependent services, assess credential risk, draft safe human-executed rotation procedures, and stop at a review gate without mutating external systems.
...
- Do not hardcode `/Users/gregmushen/work/code/taskrail` or any absolute checkout path in queue YAML, prompts, fixtures, specs, or implementation code.
