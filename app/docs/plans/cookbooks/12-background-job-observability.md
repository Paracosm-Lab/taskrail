...
**Goal:** Add the `job_observability` cookbook queue so TaskRail can scan async job classes, score their observability, draft missing instrumentation fixes, and gate the results through tests and human review.
...
This cookbook assumes the shared TaskRail development/test infrastructure is already available. It does not define new Docker Compose services. External services in the fixture app are fake Ruby classes so the cookbook can run in local and Docker-friendly test environments.
...
```bash
! grep -R "/Users/gregmushen/work/code/taskrail" config/queues/job_observability.yml prompts/jobs_*.md test/fixtures/apps/uninstrumented_jobs docs/cookbooks/background-job-observability.md
