...
**Alert:** `service-down`
**Severity:** critical
**Fires when:** `up{service="taskrail"} == 0`
**Meaning:** The TaskRail API is unavailable to clients.
...
```bash
curl -fsS https://taskrail.example.com/health
```
...
1. Enable maintenance mode.
   ```bash
   curl -fsS -X PUT https://taskrail.example.com/admin/maintenance \
     -H "Authorization: Bearer $TASKRAIL_ADMIN_TOKEN" \
...
**Alert:** `queue-stalled`
**Severity:** warning
**Fires when:** `taskrail_work_items_stalled_total > 0`
...
```bash
curl -fsS https://taskrail.example.com/api/v1/work_items?status=in_progress \
  -H "Authorization: Bearer $TASKRAIL_SERVICE_TOKEN"
```
